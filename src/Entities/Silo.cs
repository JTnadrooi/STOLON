...
namespace STOLON
{
    public class SiloEntity : Entity
    {
        public SiloEntity() : base("silo", "Silo", "Sl", new EntityProfile("silo", new Point(245, 180)),
            [
                new EntityNote("Gains 20% allocation when Deceit is included in the selection.", i => i.IsSelected("deceit"), EntityNotePolarity.Positive),
                new EntityNote("Loses 50% allocation when more than 3 entities are included in the selection.", i => i.Entries.Count > 3, EntityNotePolarity.Positive),
            ], "Silo", "Silo 28SHA")
        {
        }
